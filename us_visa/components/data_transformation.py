...
class DataTransformation:
    ...
    def get_data_transformer_object(self) -> Pipeline:
        ...
        try:
            ...
            logging.info("Initialized StandardScaler, OneHotEncoder, OrdinalEncoder, PolynomialFeatures")
    ...
    def initiate_data_transformation(self) -> DataTransformationArtifact:
        try:
            if self.data_validation_artifact.validation_status:
                ...
                stats_analysis = {
                    "train_mean": np.mean(input_feature_train_final, axis=0).tolist(),
                    "train_std": np.std(input_feature_train_final, axis=0).tolist(),
                    "test_mean": np.mean(input_feature_test_final, axis=0).tolist(),
                    "test_std": np.std(input_feature_test_final, axis=0).tolist(),
                }

                # Ensure the directory exists
                os.makedirs(os.path.dirname(self.data_transformation_config.stats_analysis_file_path), exist_ok=True)

                # Save the statistical analysis as a JSON file
                with open(self.data_transformation_config.stats_analysis_file_path, 'w') as f:
